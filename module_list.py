...
def compute_icc_loss(rep, label, mask, prob, strong_threshold=1.0, temp=0.5, num_queries=256, num_negatives=256):
    ...
    if len(seg_num_list) <= 1:  
        ...
        for i in range(valid_seg):
            ...
            icc_loss = icc_loss + F.cross_entropy(seg_logits / temp, torch.zeros(num_queries).long().to(device))
        return icc_loss / valid_seg
...
class ConfMatrix(object):
    ...
    def get_metrics_test(self):
        ...
        return torch.mean(iu).item(), torch.mean(acc).item(), iu, macro_precision, macro_recall, macro_f1
