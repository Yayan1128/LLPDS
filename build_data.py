...
# --------------------------------------------------------------------------------
def transform_u(image, label, logits=None, crop_size=(512, 512), scale_size=(0.8, 1.0), augmentation=True):
    ...
    if augmentation:
        ...
        if torch.rand(1) > 0.2:
            ...
            color_transform = transforms.ColorJitter((0.75, 1.25), (0.75, 1.25), (0.75, 1.25),
                                                     (-0.25, 0.25))  # For PyTorch 1.9/TorchVision 0.10 users
            # color_transform = transforms.ColorJitter.get_params((0.75, 1.25), (0.75, 1.25), (0.75, 1.25), (-0.25, 0.25))
...
def get_pascal_idx(root, train=True, label_num=5):
    ...
    if train:
        labels = []
        unlabels = []
        file_name_label = root + 'train_supervised.txt'
        file_name_unlabel = root + 'train_unsupervised.txt'
        with open(file_name_label) as f:
            files = f.readlines()
            for item in files:
                name = item.strip()
                labels.append(name)
        with open(file_name_unlabel) as f:
            files = f.readlines()
            for item in files:
                name = item.strip()
        ...
        with open(file_name_val) as f:
            ...
            for item in files:
                name = item.strip()
                vals.append(name)
        file_name_test = root + 'test.txt'
        test = []
        with open(file_name_test) as f_t:
            files_t = f_t.readlines()
            for item in files_t:
                name = item.strip()
                test.append(name)
...
class BuildDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset == 'sun':
            ...
            image, label = transform(image_root, label_root, None, self.crop_size, self.scale_size, self.augmentation)
            return image, label.squeeze(0)
        if self.dataset == 'crack':
            sp =  self.idx_list[index].strip().split(' ')
            # print(sp)
            # self.idx_list[index]=self.idx_list[index].strip().spilt(' ')
            # img = np.array(cv2.imread(self.root +sp[0], cv2.IMREAD_COLOR))
            image_root = Image.open(self.root +sp[0])
                                    # '{}.jpg'.format(self.idx_list[index][0]))
            # if self.apply_partial is None:
            #     label_root = Image.open(self.root + '/SegmentationClassAug/{}.png'.format(self.idx_list[index]))
            # else:
            image_id = sp[0].split("/")[-1].split(".")[0]
            mask = np.asarray(Image.open(self.root + sp[1]),dtype=np.int32)
            img = np.array(image_root)

            # mask = np.array(label_root)
            # print(mask.max())
            ##crack500数据集
            # print(img.shape)
            mas_h, mas_w, _ = img.shape
            # print(img.shape)
            if mas_h == 640:
                img = np.rot90(img)
                mask = np.rot90(mask)
            ...
            # print(img.sum())
            # print(mask.max())
            image_root = Image.fromarray(img)
            label_root = Image.fromarray(mask)
            # print('aa')
            # print(image_root)
            image, label = transform(image_root, label_root, None, self.crop_size, self.scale_size, self.augmentation)
...
class BuildDataLoader:
    def __init__(self, dataset, num_labels):
        ...
        if dataset == 'crack':
            self.im_size = [360, 640]
            self.crop_size = [360, 640]
            self.num_segments = 2
            self.scale_size = (0.5, 1.5)
            # self.scale_size = (1, 1)
            self.batch_size = 2
