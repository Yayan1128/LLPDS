...
class SlidingEval(torch.nn.Module):
    def __init__(self, model, crop_size, stride_rate, device, class_number=2, val_id=1):
    ...
    def scale_process(self, img, ori_shape, device=None):
        ...
        if long_size <= min(360, 640):###
            ...
            with torch.no_grad():
                ...
                if self.val_id == 1:
                    score = self.model.module.decoder1(self.model.module.encoder1(input_data),
                                                                    data_shape=[input_data.shape[-2], input_data.shape[-1]])

                    # score = self.model.module(input_data, id=1)
                else:
                    score1= self.model.module.decoder1(self.model.module.encoder1(input_data),
                                                       data_shape=[input_data.shape[-2], input_data.shape[-1]])
                    score2 = self.model.module.decoder2(self.model.module.encoder2(input_data),
                                                        data_shape=[input_data.shape[-2], input_data.shape[-1]])
        ...
        # print(score.size())
        # score = score.squeeze(1).permute(1, 2, 0)#
        score = score.squeeze(0).permute(1, 2, 0)
        # print(score.shape)
